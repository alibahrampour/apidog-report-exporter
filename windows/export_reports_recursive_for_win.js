// export_reports_recursive.js - FINAL WINDOWS OPTIMIZED VERSION
...
const outputDir = process.env.APIDOG_OUTPUT_DIR ||
    path.join(process.env.USERPROFILE, 'Documents', 'ApiDogReport');
// ====================================================================
...
function walkDir(dir) {
  ...
  try {
      ...
      // Ignores folders that are inaccessible or have been deleted.
      // console.error('walk error for', dir, e.message); 
  }
  return results;
}
...
function generateCurlCommand(data) {
    ...
    // Use double quotes (") on Windows for more compatibility
    ...
    for (const header of headers) {
        ...
        // In Windows, ^ (caret) is used to break a line in the Command Prompt
        // Also for data we use -d or --data instead of --data-raw
    ...
    if (body) {
        ...
        if (bodyContent) {
            // Escape quotes in Buddy for Windows
            const escapedBody = bodyContent.replace(/"/g, '\\"');
            // On Windows, use -d or --data
            curl += ` ^\n  --data "${escapedBody}"`;
        }
    }
...
function main() {
  try {
    ...
    if (!fs.existsSync(baseDir)) {
      console.error('❌ Base apiTestReport folder not found at:', baseDir);
      console.error('Please make sure Apidog is installed and the project ID (project-767820) is correct.');
      process.exit(1);
    }
    ensureOutDir();
